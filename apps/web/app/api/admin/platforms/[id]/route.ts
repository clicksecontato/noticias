...
export async function PATCH(
  ...
  let body: { slug?: string; name?: string; vendor?: string | null };
  try {
  ...
  try {
    ...
    if (!row) return Response.json({ error: "Plataforma não encontrada" }, { status: 404 });
    const updated = await catalogRepository.updatePlatform(id, {
