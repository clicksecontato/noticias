...
export async function GET(
  ...
  try {
    ...
    if (!row) return Response.json({ error: "Assunto não encontrado" }, { status: 404 });
...
export async function PATCH(
  ...
  try {
    const row = await catalogRepository.updateSubject(id, {
      slug: body.slug,
      name: body.name,
...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    await catalogRepository.deleteSubject(id);
